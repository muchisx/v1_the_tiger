...
function Button(props: Props) {
  ...
  // 2️⃣ Animation handler for when both text and icon are present
  // -------------------------- --------------------------
  const [isHovered, setIsHovered] = useState<boolean>(false);
  const isHoveredRef = useRef<boolean>(false);
  const timeoutRef = useRef<number>();

  const handleTextAndIconHover = () => {
  ...
  // 3️⃣ Animation handler for when there's only one child (Text or Icon), but not both
  // -------------------------- --------------------------
  const singleChildAnimation = (buttonIcon?: Props['Icon'], buttonText?: Props['text']) => {
    const animation =
  ...
  return (
    <Container
      ...
      onMouseEnter={handleTextAndIconHover}
      onMouseLeave={handleTextAndIconHover}
      whileHover={singleChildAnimation(Icon, text)}
      onClick={action}
      className={className}
      isHovered={isHovered}
    >
